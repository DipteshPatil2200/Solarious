...
export default function AdminDashboard({ adminName }:{ adminName:string }) {
  const [tab,setTab]=useState<"inquiries"|"products"|"resources"|"homepage">("inquiries");
  ...
  return <main className="admin-page">
    ...
    </section>:tab==="products"?<section className="admin-panel"><div className="admin-panel-head"><div><h2>Product catalogue</h2><p>Edit the product information shown across the website.</p></div></div><div className="admin-product-grid">{products.map(product=><article key={product.slug}><img src={product.image} alt=""/><div><span>{product.technology}</span><h3>{product.name}</h3><p>{product.summary}</p><button className="button button-ghost" onClick={()=>setEditing({...product})}>Edit product</button></div></article>)}</div></section>:tab==="resources"?<AdminResources/>:<AdminHeroBanners/>}
